  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--w-bg)', fontFamily: 'Inter, sans-serif', color: 'var(--w-text-1)', overflowX: 'hidden' }}>
          ...
          {STEPS.map((step, i) => (
            <div key={i} style={{
              ...
              >
                ...
                <h3 style={{ fontSize: 22, fontWeight: 800, fontFamily: 'Outfit, sans-serif', color: 'var(--w-text-1)', marginBottom: 12, letterSpacing: '-0.01em' }}>
                  {step.title}
                </h3>
                <p style={{ fontSize: 15, color: 'var(--w-text-2)', lineHeight: 1.8, marginBottom: 16 }}>
                  {step.desc}
                </p>
                <div style={{
                  fontSize: 13, color: 'var(--w-text-3)', lineHeight: 1.7,
      ...
      <section style={{
        ...
        borderBottom: '1px solid var(--w-card-border)', padding: '96px 48px',
      }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 64 }}>
            ...
            <p style={{ fontSize: 16, color: 'var(--w-text-3)', maxWidth: 500, margin: '0 auto' }}>See how PayGuard transforms fraud outcomes across common attack vectors.</p>
          </div>
            ...
            {SCENARIOS.map((s, i) => (
              <div key={i} style={{
                ...
                <div style={{
                  ...
                  <p style={{ fontSize: 14, color: 'var(--w-text-2)', lineHeight: 1.75 }}>{s.without}</p>
                </div>
                ...
                <div style={{ background: 'rgba(63,185,80,0.04)', padding: '28px 32px' }}>
                  ...
                  <p style={{ fontSize: 14, color: 'var(--w-text-2)', lineHeight: 1.75 }}>{s.with}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <section style={{ padding: '96px 48px', maxWidth: 900, margin: '0 auto' }}>
        <div style={{ textAlign: 'center', marginBottom: 64 }}>
          ...
          <p style={{ fontSize: 16, color: 'var(--w-text-3)' }}>Three steps from first install to production-grade fraud protection.</p>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 20 }}>
          {INTEGRATION_STEPS.map((s, i) => (
            <div key={i} style={{
              ...
              <h3 style={{ fontSize: 18, fontWeight: 800, color: 'var(--w-text-1)', fontFamily: 'Outfit, sans-serif', marginBottom: 10 }}>{s.title}</h3>
      ...
      <section style={{
        background: 'rgba(255,255,255,0.015)', borderTop: '1px solid rgba(255,255,255,0.05)',
        borderBottom: '1px solid var(--w-card-border)', padding: '80px 48px',
      }}>
        <div style={{ maxWidth: 1000, margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 24 }}>
          {[
            ...
            <div key={i} style={{ textAlign: 'center' }}>
              ...
              <div style={{ fontSize: 13, color: 'var(--w-text-3)' }}>{s.label}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      <section style={{ padding: '96px 48px', textAlign: 'center' }}>
        ...
        <p style={{ fontSize: 16, color: 'var(--w-text-3)', marginBottom: 36, maxWidth: 500, margin: '0 auto 36px' }}>
